...
def a_star(successors, start_state, goal_test, heuristic=lambda x: 0):
	if goal_test(start_state, time.time()):
		return [start_state]
	start_node = SearchNode(start_state, None, time.time()+planning_time ,0)
...
class system:
	...
	def find_path(self, ID1, end_ID):
		...
		def goal_test(state, time):
			...
			#print(id)
			if end_ID == id:
				safe_intervals = si_dict[id]
				for interval in safe_intervals:
					if interval[0] + space_time < time < interval[1] - (space_time+escape_time):
						return  True
			return(False)
		def dist(state):
			id = state#[0]
