//spohorec 20160608 added extra text to error messages to try to find the src of an error. No luck. 
...
Crazyradio::~Crazyradio()
{
    ...
    if (result != LIBUSB_SUCCESS) {
        std::cerr <<  "Error 0" << libusb_error_name(result) << std::endl;
    }
...
bool Crazyradio::open(uint32_t devid)
{
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (err != LIBUSB_SUCCESS) {
            std::cerr << "Error 1" << libusb_error_name(err) << std::endl;
            libusb_free_device_list(list, 1);
            return false;
        }
        else if (deviceDescriptor.idVendor == 0x1915 &&
                 deviceDescriptor.idProduct == 0x07777) { //changed from 7777 since Nordic Semiconductor ASA has that product ID.
            if (foundid == devid) {
                found = device;
                break;
            }
    ...
    if (found) {
        ...
        if (err != LIBUSB_SUCCESS) {
            std::cerr << "id is: " << found << "Error 2" << libusb_error_name(err) << std::endl;
            libusb_free_device_list(list, 1);
            return false;
        }
        libusb_device_descriptor deviceDescriptor;
        ...
        if (err != LIBUSB_SUCCESS) {
            std::cerr <<  "Error 3" << libusb_error_name(err) << std::endl;
            return false;
        }
    ...
    if (m_handle)
    {
        ...
        if (err != LIBUSB_SUCCESS) {
            std::cerr <<  "Error 4" << libusb_error_name(err) << std::endl;
            return false;
        }

        err = libusb_claim_interface(m_handle, 0);
        if (err != LIBUSB_SUCCESS) {
            std::cerr <<  "Error 5" << libusb_error_name(err) << std::endl;
            return false;
        }
